...
class NewsFromMinasTirith(Node):
    def __init__(self):
        super().__init__("news_from_minas_tirith_node")
    ...
    def callback_for_publishing(self):
        msg = String()
        msg.data = "MINAS TIRITH: Do you guys know where Boromir is?"
        self.publisher_.publish(msg=msg)

def main(args=None):
    rclpy.init(args=args)
    the_node = NewsFromMinasTirith()
    rclpy.spin(the_node)
    rclpy.shutdown()
